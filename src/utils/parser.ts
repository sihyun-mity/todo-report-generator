...
const ANY_BULLET = `[${PROJECT_BULLETS}${TASK_BULLETS}]`;
...
const restoreLineBreaks = (text: string) =>
  text
    ...
    .replace(new RegExp(`([%％][ \\t]*[)\\]）]?)[ \\t]*(?=${ANY_BULLET}[ \\t])`, 'g'), '$1\n')
    // ③ 공백 2칸 이상 뒤에 오는 불릿은 원래 줄바꿈 + 들여쓰기였다고 본다. 들여쓰기 폭은 보존한다.
    .replace(new RegExp(`([ \\t]{2,})(?=${ANY_BULLET}[ \\t])`, 'g'), '\n$1');
...
const toTask = (raw: string): Task | null => {
  ...
  return { id: createId(), content, progress };
};
...
export const parseReportText = (text: string): ParsedReport | null => {
  ...
  for (const line of lines) {
    ...
    if (currentProject && indent > currentProjectIndent) addTask(trimmed);
